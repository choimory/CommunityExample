...
public interface ShopService {
	  ...
	  public List<ShopDTO> iconList(ShopDTO shopDto, PageHelper pageHelper); //아이콘의 글들을 page를 이용해 가져오기
	  public Map<String, Object> content(String stringShopIdx); //해당 idx의 정보와 댓글들 가져오기
	...
	//  public ? wishList(?); //해당아이템 북마크 등록유무 검색후 없으면 등록 있으면 삭제
	  ...
	  public int thumbsUpContent(ShopDTO shopDto); //sIdx글 추천수증가, 해당글 추천mId 등록
	  public int thumbsDownContent(ShopDTO shopDto); //sIdx글 추천수감소, 해당글 추천mId 삭제
	  public int thumbsUpComment(ShopCommentDTO shopCommentDto); //scIdx댓글 추천수증가, 해당댓글 추천mId 등록
	  public int thumbsDownComment(ShopCommentDTO shopCommentDto); //scIdx댓글 추천수감소, 해당댓글 추천mId 제거
