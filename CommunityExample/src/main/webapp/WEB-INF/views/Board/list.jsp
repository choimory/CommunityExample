...
<div class="container bg-white col-lg-8 p-4">
	...
	<div class="board_info mb-5">
		<span class="h3 mr-2"><a
			href="/communityexample/${boardTypeDto.bId}"
	...
	<div class="ce_board_list_category">
		<c:forEach items="${boardCategoryList}" var="boardCategory">
			<button type="button" class="btn btn-info mb-1"
				onclick="location.href='/communityexample/${boardTypeDto.bId}?bCategory=${boardCategory}'">${boardCategory}</button>
		</c:forEach>
	</div>
					...
					<tr>
						...
						<c:choose>
							<c:when test="${searchHelper.query ne null}">
								<td>
									<a class="text-dark" href="/communityexample/${boardDto.bId}/${boardDto.bIdx}?query=${searchHelper.query}&title=${searchHelper.target}">${boardDto.bTitle}
							...
							<c:when test="${searchHelper.bCategory ne 'all'}">
								<td>
									<a class="text-dark" href="/communityexample/${boardDto.bId}/${boardDto.bIdx}?bCategory=${searchHelper.bCategory}">${boardDto.bTitle}
										<span class="badge badge-info mx-1">${boardDto.boardInfoDto.bCommentNum}</span>
									</a>
								</td>
							</c:when>
							<c:otherwise>
								<td>
									<a class="text-dark" href="/communityexample/${boardDto.bId}/${boardDto.bIdx}">${boardDto.bTitle}
										<span class="badge badge-info mx-1">${boardDto.boardInfoDto.bCommentNum}</span>
									</a>
								</td>
							</c:otherwise>
		...
		<nav aria-label="Page navigation example">
			<ul class="pagination justify-content-center">
				<li class="page-item"><a class="page-link"
					href="/communityexample/${boardTypeDto.bId}?page=${pageHelper.page-1}"
					...
					<c:choose>
						<c:when test="${pageHelper.page eq page}">
							<li class="page-item active"><a class="page-link"
								href="/communityexample/${boardTypeDto.bId}?page=${page}">${page}</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="/communityexample/${boardTypeDto.bId}?page=${page}">${page}</a></li>
